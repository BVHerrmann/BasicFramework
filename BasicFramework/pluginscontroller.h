...
public:
    explicit PluginsController(BasicFramework*mainWindow, QObject *parent = 0);
    ...
    AlarmDatabase *alarmDatabase() const { return _alarmDatabase; }
...
public slots:
    // PluginInterface
    void setRequestedMachineState(PluginInterface::MachineState state)
    {
        _requested_machine_state = state;
        emit requestedMachineStateChanged(state); 
    }
